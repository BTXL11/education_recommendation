...
def generate_G_from_H(H, variable_weight=False):
    ...
    invDE = np.diag(np.power(DE, float(-1)))    # Inverse degree of edges
    DV2 = np.diag(np.power(DV, -0.5))    # Degree normalization for vertices

    W = np.diag(W)    # Weighted adjacency
    H = np.array(H)
...
def sparse_mx_to_torch_sparse_tensor(sparse_mx):
    ...
    return torch.sparse_coo_tensor(indices, values, shape)    # Return sparse tensor
