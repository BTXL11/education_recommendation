...
### 优点:
- 直观易懂: 协同过滤算法的原理简单，易于理解。
- 潜在兴趣发现: 能够发现用户的潜在兴趣，推荐结果往往具有惊喜性。
- 快速启动: 基于物品的协同过滤算法在处理新用户时更为高效，能够快速为用户提供个性化推荐。 

### 缺点:
- 无法推荐新物品: 特别是在基于物品的算法中，新物品没有评分，难以进入推荐系统。
- 数据需求: 需要大量的用户数据来构建准确的用户偏好模型。
- 算法健壮性: 需要更多的数据来验证和优化算法。 
- 冷启动问题: 用户数量大时计算量大，冷启动问题严重。

- #### 冷启动问题
   简单说：系统刚上线/刚有新用户/新物品时，没有历史数据，没法正常推荐、预测或运行，这种“从零开始、没数据可用”的困境，就叫冷启动。

   常见在三个场景：
 
  1. 推荐系统里最典型
 
  - 用户冷启动：新用户刚注册，没有浏览、点击记录，不知道该推什么给他。
​
  - 物品冷启动：新上架商品、新歌、新视频，没人互动过，没法判断谁会喜欢。
​
  - 系统冷启动：产品刚上线，完全没数据，整个推荐逻辑跑不起来。
 
  2. 机器学习/模型训练
 
  - 新场景、新任务，没有标注数据或样本极少，模型无法训练或效果极差。
 
  3. 产品/业务层面
 
  - 新产品刚推出，没有用户、没有流量、没有口碑，早期很难冷启动增长。


## 对协同过滤算法的修正

### 1. 基于用户特征权重置信度的协同过滤推荐算法 ---基于改进的协同过滤算法在课程资源推荐系统的应用_刘昊东
